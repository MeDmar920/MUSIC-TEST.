...
client.on('ready', () => {
    console.log(`Created By: Toxic Codes`);
    console.log(`Developed By: • Ṫ 〤 2oLF🎤`);
    console.log(`Guilds: ${client.guilds.size}`);
    console.log(`Users: ${client.users.size}`);
    client.user.setActivity('Music Bot',{type: 'WATCHING'});
});
...
const devs = ['462006869834203159','583003275012341780'];

client.on('message', message => {
    let argresult = message.content.split(` `).slice(1).join(' ');
    if (message.content.startsWith(prefix + 'setStreaming')) {
      if (!devs.includes(message.author.id)) return message.channel.send("<@583003275012341780> only this guy can do restart the bot so don't try again :wink:.");
      message.delete();
      client.user.setGame(argresult, 'https://twitch.tv/Toxic Codes');
...
client.login("NTk3ODAyMTkyNDA5MzI5Njc1.XSmpIA.0Nk_3hEPldOU03GQZ3ZNwMpvkvc");
